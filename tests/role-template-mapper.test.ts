...
describe('role template mapper', () => {
  test('maps /getRoles privileges while preserving backend privilege keys', () => {
    ...
    assert.equal(roles[0].privileges.master_account, 1);
    assert.equal(roles[0].privileges.master_counter, 0);
  ...
  test('builds create-user privileges from edited permissions using selected role key casing', () => {
    ...
    assert.equal(payload.master_account, 1);
    assert.equal(payload.master_counter, 1);
    assert.equal(payload.customers_add, 0);
    assert.equal(payload.master_inventory, 0);
    assert.equal(payload.accounts_cash_deposit, 0);
    assert.equal(payload.accounts_balance_transfer, 0);
    assert.equal(payload.reports_daily, 0);
  });
});
