...
const errorResponse = (error: unknown) => {
  ...
  return Response.json({ success: false, message: 'Unable to load employees.' }, { status: 502 });
};

export async function GET() {
  const endpoint = process.env.NEXT_PUBLIC_API_EMPLOYEES_PATH?.trim();
  if (!endpoint) {
    return pendingResponse();
  }

  try {
    // Token is httpOnly, so configured employee backend calls run server-side.
