...
export async function GET() {
  try {
    return Response.json(await backendFetch('customer-colors', { method: 'GET' }));
  } catch (error) {
    return errorResponse(error, 'Unable to load customer colors.');
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    return Response.json(await backendFetch('customer-colors', { method: 'POST', body }));
  } catch (error) {
    return errorResponse(error, 'Unable to save customer color.');
  }
}
