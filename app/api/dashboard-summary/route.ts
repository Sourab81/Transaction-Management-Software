...
const errorResponse = (error: unknown) => {
  ...
  return Response.json({ success: false, message: 'Unable to load dashboard summary.' }, { status: 502 });
};
...
export async function GET() {
  ...
  try {
    // Token is httpOnly, so configured dashboard summary calls run server-side.
